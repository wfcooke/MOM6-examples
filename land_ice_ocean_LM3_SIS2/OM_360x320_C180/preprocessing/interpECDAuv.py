...
for n in np.arange(0,1):
   O=state('/archive/fjz/CM2.1R/CM2.1R_ECDA_v3.1_1960/gfdl.default-prod/pp_ensemble/ocean/av/annual_56yr/ocean.1961-2016.ann.nc',fields=['u','v'],time_indices=np.arange(n,n+1),default_calendar='noleap',z_orientation=-1)
   ...
   #OM.remap_ALE(fields=['uo'],z_bounds=zb,zbax_data=-zi,method='ppm_h4',bndy_extrapolation=False)
   ...
   if n==0:
      OM.write_nc('ECDA_u_monthly.nc',['uo','vo'],append=False,write_interface_positions=True)
   else:
      OM.write_nc('ECDA_u_monthly.nc',['uo','vo'],append=True,write_interface_positions=True)
